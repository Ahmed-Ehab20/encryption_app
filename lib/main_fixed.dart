          ...
          children: [
            Row(
              children: [
                ...
                SizedBox(width: 8),
                Text(
                  widget.mode == 'encrypt' ? 'Plaintext' : 'Ciphertext',
                  style: TextStyle(
                    fontSize: 16,
                    fontWeight: FontWeight.bold,
                    color: isDark ? Colors.white : Colors.black87,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text(
              _errorMessage.isEmpty ? 'Analyzing ciphertext...' : _errorMessage,
              textAlign: TextAlign.center,
              style: TextStyle(
                color: Theme.of(context).primaryColor,
                fontWeight: FontWeight.w500,
        ...
        children: [
          ...
          ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              final isKeySaved = _savedKeys.contains(key);
                        ...
                        children: [
                          ...
                          Row(
                            ...
                            children: [
                              if (!isKeySaved)
                                OutlinedButton.icon(
                                  icon: Icon(Icons.bookmark_add, size: 16),
                                  label: Text('Save Key'),
                                  onPressed: () {
                                    setState(() {
                                      _savedKeys.add(key);
                                    });
                                    _updateSavedKeys(); // Add this to update parent
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar(
                                        content: Text(
                                            'Key "$key" saved for future use'),
                                        duration: Duration(seconds: 2),
                                      ),
                                    );
                                  },
                                ...
                                OutlinedButton.icon(
                                  ...
                                  onPressed: () {
                                    ...
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar(
                                        content: Text(
                                            'Key "$key" removed from saved keys'),
                                        duration: Duration(seconds: 2),
                                      ),
                                    );
                                  },
                              ...
                              ElevatedButton.icon(
                                ...
                                onPressed: () {
                                  Clipboard.setData(
                                      ClipboardData(text: result['decrypted']));
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                              ...
                              OutlinedButton.icon(
                                ...
                                label: Text('Use This'),
                                onPressed: () {
                                  _controller.text = result['decrypted'];
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content:
                                          Text('Result applied to input field'),
                                      duration: Duration(seconds: 2),
                                    ),
                                  );
                                },
